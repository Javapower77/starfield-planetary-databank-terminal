...
String ScriptNameString = "MissionParentScript"
...
Function ResetMissions(Bool bShutDownUnstarted, Bool bShutDownAll, Location resetLocation, Bool resetLocationOverride)
  ...
  Javas_Debug.WriteLogFunction(ScriptNameString, "IN", "ResetMissions(Bool bShutDownUnstarted, Bool bShutDownAll, Location resetLocation, Bool resetLocationOverride)", 1, 2)
  Javas_Debug.WriteLog(ScriptNameString, "ResetMissions", "Bool bShutDownUnstarted", bShutDownUnstarted, 1, 3)
  Javas_Debug.WriteLog(ScriptNameString, "ResetMissions", "Bool bShutDownAll", bShutDownAll, 1, 3)
  Javas_Debug.WriteLog(ScriptNameString, "ResetMissions", "Location resetLocation", resetLocation, 1, 3)
  Javas_Debug.WriteLog(ScriptNameString, "ResetMissions", "Bool resetLocationOverride", resetLocationOverride, 1, 3)  
  Guard resetMissionGuard ;*** WARNING: Experimental syntax, may be incorrect: Guard  ; #DEBUG_LINE_NO:212
    If resetLocation != lastResetLocation || resetLocationOverride ; #DEBUG_LINE_NO:215
      Javas_Debug.WriteLogComment(ScriptNameString, "ResetMissions", "The condition resetLocation is not equal than lastResetLocation and resetLocationOverride is True, so then trigger the Story Manager.", 1, 3)
      lastResetLocation = resetLocation ; #DEBUG_LINE_NO:216
      lastResetTimestamp = Utility.GetCurrentGameTime() ; #DEBUG_LINE_NO:217
      Javas_Debug.WriteLogComment(ScriptNameString, "ResetMissions", "Start the Timer for Game Time -> Self.StartTimerGameTime(resetTimeInterval[=" + resetTimeInterval as String + "] * 24.0, resetTimerID[=" + resetTimerID as String + "])", 1, 3)
      Self.StartTimerGameTime(resetTimeInterval * 24.0, resetTimerID) ; #DEBUG_LINE_NO:218
      Javas_Debug.WriteLogComment(ScriptNameString, "ResetMissions", "Stop Missions -> Self.StopMissions(bShutDownUnstarted[=" + bShutDownUnstarted as String + "], bShutDownAll[=" + bShutDownAll as String + "])", 1, 3)
      ...
      Javas_Debug.WriteLog(ScriptNameString, "ResetMissions", "MissionTypes.Length", MissionTypes.Length, 1, 3)
      Javas_Debug.WriteLogComment(ScriptNameString, "ResetMissions", "Go through all Mission Types and trigger the Story Manager using the Keyword on theMissionType.missionTypeKeyword to know which one must run.", 1, 3)  
      While I < MissionTypes.Length ; #DEBUG_LINE_NO:227
        ...
        Javas_Debug.WriteLog(ScriptNameString, "ResetMissions", "theMissionType.missionTypeKeyword", theMissionType.missionTypeKeyword + " Random: " + theMissionType.RandomStoryEventOrder, 1, 3)
        If theMissionType.RandomStoryEventOrder ; #DEBUG_LINE_NO:229
          Javas_Debug.WriteLogComment(ScriptNameString, "ResetMissions", "Add Mission Type to Random Story Event Array", 1, 3)
          randomMissionEvents.add(theMissionType.missionTypeKeyword, 1) ; #DEBUG_LINE_NO:230
        Else
          Javas_Debug.WriteLogComment(ScriptNameString, "ResetMissions", "Send Mission Type to the Story Event", 1, 3)
      ...
      While randomMissionEvents.Length > 0 ; #DEBUG_LINE_NO:238
        ...
        Javas_Debug.WriteLogComment(ScriptNameString, "ResetMissions", "Send Random Mission Type " + randomMissionEvents[randomIndex] + "to the Story Event", 1, 3)
    ...
    EndIf
  EndGuard ;*** WARNING: Experimental syntax, may be incorrect: EndGuard 
  Javas_Debug.WriteLogFunction(ScriptNameString, "OUT", "ResetMissions(Bool bShutDownUnstarted, Bool bShutDownAll, Location resetLocation, Bool resetLocationOverride)", 1, 2)
EndFunction
